...
class randomList:
    ...
    def capitalLetters(returns=1):
        ...
        lists = []
        for i in range(returns):
            lists.append(choice(list("АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ")))
        return lists
    ...
    def letters(returns=1):
        ...
        lists = []
        for i in range(returns):
            lists.append(choice(list("абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ")))
        return lists
